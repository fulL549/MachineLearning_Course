...
class DrawingApplication:
    ...
    def create_menu(self):
        ...
        # 方向键微移（全局绑定，避免焦点不在画布时失效）
        self.root.bind('<Up>', lambda e: self.canvas.nudge_selected(0, -10))
        self.root.bind('<Down>', lambda e: self.canvas.nudge_selected(0, 10))
        self.root.bind('<Left>', lambda e: self.canvas.nudge_selected(-10, 0))
        self.root.bind('<Right>', lambda e: self.canvas.nudge_selected(10, 0))
    ...
    def create_toolbar(self):
        ...
        tk.Button(
            action_frame,
            text="⬆ 上移",
            command=lambda: self.canvas.nudge_selected(0, -10),
            bg=PALETTE["bg_button"],
            activebackground=PALETTE["bg_button_accent"],
        ).pack(side=tk.LEFT, padx=2)
        ...
        tk.Button(
            action_frame,
            text="⬅ 左移",
            command=lambda: self.canvas.nudge_selected(-10, 0),
            bg=PALETTE["bg_button"],
            activebackground=PALETTE["bg_button_accent"],
        ).pack(side=tk.LEFT, padx=2)
