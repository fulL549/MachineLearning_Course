...
# 统一深蓝色配色方案
PALETTE = {
    "bg_root": "#D6EAF8",       # 主窗口背景 
    "bg_toolbar": "#AED6F1",    # 工具栏背景 
    "bg_panel": "#D4E6F1",      # 左侧面板/右侧容器背景
    "bg_labelframe": "#A9CCE3",  # 分组框背景
    "bg_button": "#85C1E9",     # 普通按钮
    "bg_button_accent": "#5DADE2",  # 强调按钮/悬停
    "bg_status": "#AED6F1"      # 状态栏
}
...
class DrawingApplication:
    ...
    def create_menu(self):
        ...
        tools_menu.add_command(label="Bézier曲线工具", command=lambda: self.set_tool("bezier_curve"))
        tools_menu.add_command(label="B样条曲线工具", command=lambda: self.set_tool("bspline_curve"))
        tools_menu.add_command(label="Bézier曲面工具", command=lambda: self.set_tool("bezier_surface"))
        tools_menu.add_command(label="Hermite曲线工具", command=lambda: self.set_tool("hermite_curve"))
        tools_menu.add_command(label="Catmull-Rom曲线工具", command=lambda: self.set_tool("catmull_rom_curve"))
    ...
    def create_toolbar(self):
        """创建工具栏"""
        # 第一行工具栏：绘图工具
        toolbar_row1 = tk.Frame(self.root, bg=PALETTE["bg_toolbar"], relief=tk.RAISED, bd=1)
        toolbar_row1.pack(side=tk.TOP, fill=tk.X, padx=2, pady=1)

        # 第二行工具栏：属性配置与操作
        toolbar_row2 = tk.Frame(self.root, bg=PALETTE["bg_toolbar"], relief=tk.RAISED, bd=1)
        toolbar_row2.pack(side=tk.TOP, fill=tk.X, padx=2, pady=1)
        ...
        tools = [
            ...
            ("圆形", "circle", "⭕"),
            ("多边形", "polygon", "🔷"),
            ("Bézier曲线", "bezier_curve", "〰️"),
            ("B样条曲线", "bspline_curve", "🐍"),
            ("Bézier曲面", "bezier_surface", "🕸️"),
            ("B样条曲面", "bspline_surface", "🕸️"),
            ("Hermite曲线", "hermite_curve", "🔗"),
            ("Catmull-Rom", "catmull_rom_curve", "📎"),
            ("立方体", "cube", "🧊"),
            ("椭圆", "ellipse", "🏈")
        ]

        self.tool_var = tk.StringVar(value="select")
        tool_frame = tk.LabelFrame(toolbar_row1, text="绘图工具", bg=PALETTE["bg_toolbar"])
        tool_frame.pack(side=tk.LEFT, padx=5, pady=2, fill=tk.X)
        ...
        # --- 第二行内容 ---
        # 颜色选择
        color_frame = tk.LabelFrame(toolbar_row2, text="颜色", bg=PALETTE["bg_toolbar"])
        ...
        # 分隔符
        separator2 = ttk.Separator(toolbar_row2, orient=tk.VERTICAL)
        separator2.pack(side=tk.LEFT, fill=tk.Y, padx=5)

        # 线宽设置
        width_frame = tk.LabelFrame(toolbar_row2, text="线宽", bg=PALETTE["bg_toolbar"])
        ...
        # 分隔符
        separator3 = ttk.Separator(toolbar_row2, orient=tk.VERTICAL)
        separator3.pack(side=tk.LEFT, fill=tk.Y, padx=5)

        # 操作按钮
        action_frame = tk.LabelFrame(toolbar_row2, text="操作", bg=PALETTE["bg_toolbar"])
