...
#计算不正确块数
def incorrect_tiles(state):
    incorrect_count = 0
    for i, val in enumerate(state):
        if val == 0:  #空格不计算在内
            continue
        if val != GOAL_STATE[i]:  #如果当前值与目标状态不同
            incorrect_count += 1
...
def solve_puzzle(start_state):
    ...
    g = incorrect_tiles(start_state)  # 计算不正确块数，表示当前状态到目标状态的代价
    f = g + h  # 启发函数 f = 不正确块数 g + 已走步数 h
    ...
    while priority_queue:
        ...
        for move, new_state in get_neighbors(state):  # 拓展当前状态的所有可能的状态
            if new_state not in visited:
                new_h = h + 1  # 记录已走步数
                new_g = incorrect_tiles(new_state)  # 计算新的不正确块数
                new_f = new_h + new_g  # 计算新的 f = 已走步数 h + 不正确块数 g
