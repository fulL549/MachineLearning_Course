...
def solve_puzzle(start_state):
    ...
    g = chebyshev(start_state)  # 计算切尔雪夫距离，表示当前状态到目标状态的代价
    f = g + h  # 启发函数 f = 切尔雪夫距离 g + 已走步数 h
    ...
    while priority_queue:
        ...
        for move, new_state in get_neighbors(state):  # 拓展当前状态的所有可能的状态
            if new_state not in visited:
                new_h = h + 1  # 记录已走步数
                new_g = chebyshev(new_state)  # 计算新的切尔雪夫距离
                new_f = new_h + new_g  # 计算新的 f = 已走步数 h + 切尔雪夫距离 g
