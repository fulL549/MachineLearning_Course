...
int MemoryManager::allocatePhysicalPages(enum AddressPoolType type, const int count)
{
    ...
    if(start != -1)
        printf("Allocate %d pages.PhysicalAddress_Start = %x\n", count, start);
...
void MemoryManager::releasePhysicalPages(enum AddressPoolType type, const int paddr, const int count)
{
    ...
    printf("Release %d pages.PhysicalAddress_Start = %x\n", count, paddr);
}
...
printf("Allocate pages.Virtual Address:%x  Physical Address:%x\n", virtualAddress, physicalPageAddress);
